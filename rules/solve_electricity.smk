...
rule solve_operational_year:
    ...
    input:
        network=RESULTS + "networks/base_s_{clusters}_elec_l{ll}_{opts}.nc",
        weather_network="results/" + config["run"]["prefix"] + "/{operational_year}/networks/base_s_{clusters}_elec_l{ll}_{opts}.nc",
    output:
        network=RESULTS + "networks/{operational_year}_base_s_{clusters}_elec_l{ll}_{opts}.nc",
        load_shedding = RESULTS + "validation/{operational_year}_base_s_{clusters}_elec_l{ll}_{opts}.csv",
    log:
        solver=normpath(
            RESULTS
            + "logs/solve_operational_year/{operational_year}_base_s_{clusters}_elec_l{ll}_{opts}_solver.log"
        ),
        python=RESULTS
        + "logs/solve_operational_year/{operational_year}_base_s_{clusters}_elec_l{ll}_{opts}_python.log",
    benchmark:
        (
            RESULTS
            + "benchmarks/solve_operational_year/{operational_year}_base_s_{clusters}_elec_l{ll}_{opts}"
        )
    threads: solver_threads
    retries: 1
    resources:
    ...
    conda:
        "../envs/environment.yaml"
    script:
        "../scripts/solve_operational_year.py"


ruleorder: solve_network > solve_operational_year > solve_operations_network
