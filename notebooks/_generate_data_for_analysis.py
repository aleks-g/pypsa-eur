...
def average_across_years(df, years):
    ...
    list_help = [df_help.loc[i * 8760 : (i + 1) * 8760 - 1] for i in range(len(df_help) // 8760)]
    for l in list_help:
        l.index = range(8760)
...
def compute_hydro_phs_costs(opt_networks: Dict[int, pypsa.Network]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for n in opt_networks.values():
        ...
        hydro_costs.append(n.storage_units_t.p.loc[:, hydro_i] * n.storage_units_t.mu_energy_balance.loc[:, hydro_i])
        phs_costs.append(n.storage_units_t.p.loc[:, phs_i] * n.storage_units_t.mu_energy_balance.loc[:, phs_i])
...
def compute_longest_deficit(years: List[int], net_load: pd.Series) -> pd.DataFrame:
    ...
    """
    longest_deficit = pd.DataFrame(index=years, columns=["start", "end", "hours", "mean", "total"])
    for y in years:
        ...
        for i in range(1, len(helper_df.index)):
            if net_load.loc[helper_df.index[i]] > 0:
                helper_df.loc[helper_df.index[i], "deficit"] = helper_df.loc[helper_df.index[i-1], "deficit"] + 1
            else:
                helper_df.loc[helper_df.index[i], "deficit"] = 0               
        longest_deficit.loc[y, "end"] = helper_df["deficit"].idxmax()
        longest_deficit.loc[y, "start"] = helper_df["deficit"].idxmax() - pd.Timedelta(hours=helper_df["deficit"].max())
        longest_deficit.loc[y, "hours"] = helper_df["deficit"].max()
        longest_deficit.loc[y, "mean"] = helper_df.loc[longest_deficit.loc[y, "start"]:longest_deficit.loc[y, "end"]]["Net load"].mean().round(0)
        longest_deficit.loc[y, "total"] = helper_df.loc[longest_deficit.loc[y, "start"]:longest_deficit.loc[y, "end"]]["Net load"].sum().round(0)
...
def compute_system_anomaly(
    ...
    for i, row in periods.iterrows():
        ...
        load_anomaly = (total_load.loc[row["start"]:row["end"]] - (avg_load.loc[shifted_start:shifted_end].sum(axis="columns")).values)
        wind_anomaly = ((wind_cf.loc[row["start"]:row["end"]] @ wind_p) - (avg_wind.loc[shifted_start:shifted_end] @ wind_p).values)
        solar_anomaly = ((solar_cf.loc[row["start"]:row["end"]] @ solar_p) - (avg_solar.loc[shifted_start:shifted_end] @ solar_p).values)

        net_load_anomaly = load_anomaly - wind_anomaly - solar_anomaly # note the signs as net load is defined as load - wind_prod - solar_prod
        system_anomaly = pd.concat([net_load_anomaly, load_anomaly, wind_anomaly, solar_anomaly], axis="columns")
        system_anomaly.columns = ["Net load anomaly", "Load anomaly", "Wind anomaly", "Solar anomaly"]
...
def compute_used_flex(periods,
    ...
    for i, row in periods.iterrows():
        ...
        for t in disp_tech:
            used_flexibility.loc[:,t] = pd.concat(
                [used_flex[node][t] for node in nodes],
                axis="columns"
            ).sum(axis="columns").round(0)
...
def compute_net_load(opt_networks, n):
    ...
    """
    load = pd.concat([opt_networks[y].loads_t.p.sum(axis="columns") for y in opt_networks.keys()], axis=0)
    ...
    solar_gen = pd.concat([opt_networks[y].generators_t.p[solar_i].sum(axis="columns") for y in opt_networks.keys()], axis=0)
    wind_gen = pd.concat([opt_networks[y].generators_t.p[wind_i].sum(axis="columns") for y in opt_networks.keys()], axis=0)
...
def compute_flex_periods_anomalies(
    flex,
    periods,
    years
):
    ...
    periods_flex = pd.concat(flex.loc[row["start"]:row["end"]] for i, row in periods.iterrows())
    # Anomalies
    periods_anomaly_flex = anomaly_index_shift(periods_flex, avg_flex, years)
    # Peak hour and flex
    peak_flex = pd.concat([flex.loc[periods.loc[i, "peak_hour"]] for i in periods.index], axis=1,).T
...
def characteristics_sdes(
    ...
    for i, row in periods.iterrows():
        ...
        stores_periods.loc[i, "relative_discharge"] = (100 * stores_periods.loc[i, "discharge"] / (n.stores.loc[n.stores.carrier == "H2", "e_nom_opt"].sum())).round(1)
        ...
        stores_periods.loc[i, "empty"] = (n.stores_t["e"].loc[row["end"],su_i] < 0.02 * (n.stores.loc[su_i, "e_nom_opt"])).sum() / len(su_i) * 100
        ...
        stores_periods.loc[i, "max_fc_discharge"] = n.links_t.p1[fc_i].loc[row["start"]:row["end"]].abs().sum(axis="columns").max() / 1e3
        ...
        stores_periods.loc[i, "affected_fc_p_lim"] = 100 * ((n.links_t.p1[fc_i].loc[row["start"]:row["end"]].abs() > 0.99 * p_lims).sum() > 0).sum() / len(fc_i)
        stores_periods.loc[i, "ratio_p_lim"] = 100 * (((n.links_t.p1[fc_i].loc[row["start"]:row["end"]].abs() > 0.99 * p_lims).sum())/len(n.links_t.p1.loc[row["start"]:row["end"], fc_i])).mean()

        # Do the same for the two peek period before the start of the event.
        stores_periods.loc[i, "2w_prior_affected_fc_p_lim"] = 100 * ((n.links_t.p1[fc_i].loc[row["start"] - dt.timedelta(days=14):row["start"]].abs() > 0.99 * p_lims).sum() > 0).sum() / len(fc_i)
        stores_periods.loc[i, "2w_prior_ratio_p_lim"] = 100 * (((n.links_t.p1[fc_i].loc[row["start"] - dt.timedelta(days=14):row["start"]].abs() > 0.99 * p_lims).sum())/len(n.links_t.p1[fc_i].loc[row["start"] - dt.timedelta(days=14):row["start"]])).abs().mean()

        # Do the same for the two week period after the end of the event.
        stores_periods.loc[i, "2w_after_affected_fc_p_lim"] = 100 * ((n.links_t.p1[fc_i].loc[row["end"]:row["end"] + dt.timedelta(days=14)].abs() > 0.99 * p_lims).sum() > 0).sum() / len(fc_i)
        stores_periods.loc[i, "2w_after_ratio_p_lim"] = 100* (((n.links_t.p1[fc_i].loc[row["end"]:row["end"] + dt.timedelta(days=14)].abs() > 0.99 * p_lims).sum())/len(n.links_t.p1[fc_i].loc[row["end"]:row["end"] + dt.timedelta(days=14)])).abs().mean()
        ...
        stores_periods.loc[i, "recovered_battery_costs"] = batt_p.loc[row["start"]:row["end"]].sum().sum()/(batt_costs + batt_costs_e).sum()
        ...
        stores_periods.loc[i, "recovered_h2_costs"] = fc_p.loc[row["start"]:row["end"]].sum().sum()/(fc_costs + elec_costs + h2_costs).sum()
...
def gen_stack_around_sde(
    ...
    for i, period in periods.iterrows():
        ...
        gen_stack = pd.DataFrame(index = pd.date_range(start_window, end_window, freq="h"), columns = ["biomass", "nuclear", "ror", "battery discharger", "battery charger", "hydro", "PHS", "H2 fuel cell", "H2 electrolysis", "solar", "offwind", "onwind"]).astype(float)
        ...
        for tech in gen_stack.columns:
            if tech == "offwind":
                ...
                gen_stack.loc[start_window:end_window,tech] = n.generators_t.p.loc[start_window:end_window, c_id].sum(axis="columns")
            elif tech == "solar":
                ...
                gen_stack.loc[start_window:end_window,tech] = - n.links_t.p1.loc[start_window:end_window, l_id].sum(axis="columns")
            elif tech in ["H2 electrolysis", "battery charger"]:
                l_id = n.links.loc[n.links.carrier == tech].index
                gen_stack.loc[start_window:end_window,tech] = - n.links_t.p0.loc[start_window:end_window, l_id].sum(axis="columns")
            elif tech in ["PHS", "hydro"]:
                su_id = n.storage_units.loc[n.storage_units.carrier == tech].index
                gen_stack.loc[start_window:end_window,tech] = n.storage_units_t.p.loc[start_window:end_window, su_id].sum(axis="columns")
            else:
...
def nodal_flex_periods_seasonality(
    ...
    """  
    nodal_seasonality, nodal_period_flex_u, nodal_anomaly_flex_u, nodal_peak_anomaly_flex_u = {}, {}, {}, {}
    for node in nodes:
        # Seasonality of usage of flexibility per node
        nfu = nodal_flex_u_xr.sel(node = node).to_pandas().drop("node", axis="columns").astype(float).round(2)
        ...
        nodal_anomaly_flex_u[node] = anomaly_index_shift(nodal_anomaly_u, nodal_seasonality[node], years)
    ...
    nodal_seasonality = pd.concat(nodal_seasonality, axis="columns").astype(float).round(2)
    nodal_period_flex_u = pd.concat(nodal_period_flex_u, axis="index").astype(float).round(2)
    nodal_anomaly_flex_u = pd.concat(nodal_anomaly_flex_u, axis="index").astype(float).round(2)
    nodal_peak_anomaly_flex_u = pd.concat(nodal_peak_anomaly_flex_u, axis="index").astype(float).round(2)
    return nodal_seasonality, nodal_period_flex_u, nodal_anomaly_flex_u, nodal_peak_anomaly_flex_u
...
def solar_capacity_factors(opt_networks, years, winter = False):
    ...
    for year, n in opt_networks.items():
        ...
        if winter:
            df.loc[year, "solar"] = n.generators_t.p[solar_i].loc[n.generators_t.p[solar_i].index.month.isin([10, 11, 12, 1, 2, 3])].sum().sum() / (n.generators.loc[solar_i, "p_nom_opt"].sum() * 4380)
        else:
            df.loc[year, "solar"] = n.generators_t.p[solar_i].sum().sum() / (n.generators.loc[solar_i, "p_nom_opt"].sum() * 8760)
    return df
...
def stats_sde(
    ...
    for i in stats_periods.index:
        ...
        stats_periods.loc[i, "highest_net_load"] = net_load.loc[stats_periods.loc[i, "start"]:stats_periods.loc[i, "end"]].max() / 1e3
        stats_periods.loc[i, "avg_net_load"] = net_load.loc[stats_periods.loc[i, "start"]:stats_periods.loc[i, "end"]].mean()/ 1e3
        stats_periods.loc[i, "energy_deficit"] = net_load.loc[stats_periods.loc[i, "start"]:stats_periods.loc[i, "end"]].sum()/ 1e6
        stats_periods.loc[i, "duration"] = (pd.Timestamp(stats_periods.loc[i, "end"]) - pd.Timestamp(stats_periods.loc[i, "start"])).total_seconds() / 3600
        stats_periods.loc[i, "h2_discharge"] = stores_periods.loc[stores_periods.index[i], "discharge"]
        stats_periods.loc[i, "max_fc_discharge"] = stores_periods.loc[stores_periods.index[i], "max_fc_discharge"]
        ...
        helper_df = total_load.loc[start:end].values / avg_load.loc[shifted_start:shifted_end].sum(axis=1)
        helper_df = pd.DataFrame(helper_df, index = pd.date_range(start, end, freq="h"))
        stats_periods.loc[i, "avg_rel_load"] = np.mean((total_load.loc[start:end])/(avg_load.loc[shifted_start:shifted_end].sum(axis=1).values))
        ...
        stats_periods.loc[i, "recovered_battery_costs"] = stores_periods.loc[stores_periods.index[i], "recovered_battery_costs"]
        stats_periods.loc[i, "recovered_h2_costs"] = stores_periods.loc[stores_periods.index[i], "recovered_h2_costs"]
        stats_periods.loc[i, "total_costs"] = total_costs[net_y].loc[start:end].sum() / 1e9
        stats_periods.loc[i, "share_total_costs"] = stats_periods.loc[i, "total_costs"] / (total_costs[net_y].sum().sum()/1e9)
        stats_periods.loc[i, "avg_hourly_cost"] = total_costs[net_y].loc[start:end].mean() / 1e6
        stats_periods.loc[i, "normed_price_std"] = ((all_prices.loc[start:end].mean())/all_prices.loc[start:end].mean().max()).std()
...
def wind_capacity_factors(opt_networks, years, winter = False):
    ...
    for year, n in opt_networks.items():
        ...
        if winter:
            df.loc[year, "wind"] = n.generators_t.p[wind_i].loc[n.generators_t.p[wind_i].index.month.isin([10, 11, 12, 1, 2, 3])].sum().sum() / (n.generators.loc[wind_i, "p_nom_opt"].sum() * 4380)
            df.loc[year, "onwind"] = n.generators_t.p[onwind_i].loc[n.generators_t.p[onwind_i].index.month.isin([10, 11, 12, 1, 2, 3])].sum().sum() / (n.generators.loc[onwind_i, "p_nom_opt"].sum() * 4380)
            df.loc[year, "offwind"] = n.generators_t.p[offwind_i].loc[n.generators_t.p[offwind_i].index.month.isin([10, 11, 12, 1, 2, 3])].sum().sum() / (n.generators.loc[offwind_i, "p_nom_opt"].sum() * 4380)
        else:
            df.loc[year, "wind"] = n.generators_t.p[wind_i].sum().sum() / (n.generators.loc[wind_i, "p_nom_opt"].sum() * 8760)
            
            df.loc[year, "onwind"] = n.generators_t.p[onwind_i].sum().sum() / (n.generators.loc[onwind_i, "p_nom_opt"].sum() * 8760)
            
            df.loc[year, "offwind"] = n.generators_t.p[offwind_i].sum().sum() / (n.generators.loc[offwind_i, "p_nom_opt"].sum() * 8760)
    return df
...
def wind_dist(
    ...
    for year, n in opt_networks.items():
        wind_i = n.generators.loc[n.generators.carrier.str.contains("wind")].index
        wind_distr[year] = n.generators_t.p[wind_i].loc[n.generators_t.p[wind_i].index.month.isin([10, 11, 12, 1, 2, 3])].sum(axis="columns").resample("1D").mean() / n.generators.loc[wind_i, "p_nom_opt"].sum()
...
if __name__ == "__main__":
    ...
    config, scenario_def, years, opt_networks = load_opt_networks(config_name, config_str = config_str, load_networks = True)
    ...
    # Total load
    total_load = pd.concat([n.loads_t.p.sum(axis="columns") for n in opt_networks.values()])
    total_load.round(0).to_csv(f"{folder}/total_load.csv")
    nodal_load = pd.concat([n.loads_t.p.round(0) for n in opt_networks.values()], axis=0)
    nodal_load.to_csv(f"{folder}/nodal_load.csv")

    # Winter load
    winter_load = pd.DataFrame(index=years, columns = ["load"])
    for year, n in opt_networks.items():
        winter_load.loc[year, "load"] = n.loads_t.p.sum(axis="columns").loc[n.loads_t.p.index.month.isin([10, 11, 12, 1, 2, 3])].mean() / 1e3
    ...
    all_prices = pd.concat([n.buses_t.marginal_price.loc[:, price_nodes] for n in opt_networks.values()], axis=0)
    ...
    for year, n in opt_networks.items():
        s_caps.loc[year, "H2_e"] = n.stores.loc[n.stores.loc[n.stores.carrier == "H2"].index, "e_nom_opt"].sum() / 1e3 # GWh
        s_caps.loc[year, "FC_p"] = n.links.loc[n.links.loc[n.links.carrier == "H2 fuel cell"].index, "p_nom_opt"].sum() / 1e3 # GW
        s_caps.loc[year, "EL_p"] = n.links.loc[n.links.loc[n.links.carrier == "H2 electrolysis"].index, "p_nom_opt"].sum() / 1e3 # GW
    ...
    wind_i = m.generators.loc[m.generators.carrier.isin(["onwind", "offwind-ac", "offwind-dc", "offwind-float"])].index
    wind_caps = pd.concat(
    ...
    for year in years:
        highest_deficit.loc[year, "time"] = net_load.loc[f"{year}-07-01":f"{year+1}-06-30"].idxmax()
        highest_deficit.loc[year, "value"] = float(int(net_load.loc[f"{year}-07-01":f"{year+1}-06-30"].max()))
    highest_deficit.to_csv(f"{folder}/highest_net_load_deficit.csv")

    weekly_net_load = net_load.rolling(168).sum()
    max_weekly_net_load = pd.DataFrame(index=years, columns=["start", "end", "total", "mean"])
    for y in years:
        max_weekly_net_load.loc[y, "end"] = weekly_net_load.loc[f"{y}-07-01":f"{y+1}-06-30"].idxmax()
        max_weekly_net_load.loc[y, "start"] = max_weekly_net_load.loc[y, "end"] - dt.timedelta(days=7)
        max_weekly_net_load.loc[y, "total"] = weekly_net_load.loc[f"{y}-07-01":f"{y+1}-06-30"].max()
        max_weekly_net_load.loc[y, "mean"] = (weekly_net_load.loc[max_weekly_net_load.loc[y, "end"]] / 168)
    ...
    stats_periods = stats_sde(periods, stores_periods,net_load,total_load,avg_load,avg_wind, wind_cf, wind_caps, reindex_opt_objs, total_costs, all_prices)
    ...
    all_flex_detailed = pd.concat([detailed_system_flex(n) for n in opt_networks.values()])

    avg_flex_detailed, periods_flex_detailed, periods_anomaly_flex_detailed, periods_peak_flex_detailed, periods_peak_anomaly_flex_detailed = compute_flex_periods_anomalies(all_flex_detailed, periods, years)
    ...
    periods_anomaly_flex_detailed.round(3).to_csv(f"{folder}/periods_anomaly_flex_detailed.csv")
    periods_peak_flex_detailed.round(3).to_csv(f"{folder}/periods_peak_flex_detailed.csv")
    periods_peak_anomaly_flex_detailed.round(3).to_csv(f"{folder}/periods_peak_anomaly_flex_detailed.csv")
    # Coarse system flexibility
    all_flex_coarse = pd.concat([coarse_system_flex(n) for n in opt_networks.values()])
    avg_flex_coarse, periods_flex_coarse, periods_anomaly_flex_coarse, periods_peak_flex_coarse, periods_peak_anomaly_flex_coarse = compute_flex_periods_anomalies(all_flex_coarse, periods, years)
    ...
    periods_anomaly_flex_coarse.round(3).to_csv(f"{folder}/periods_anomaly_flex_coarse.csv")
    periods_peak_flex_coarse.round(3).to_csv(f"{folder}/periods_peak_flex_coarse.csv")
    periods_peak_anomaly_flex_coarse.round(3).to_csv(f"{folder}/periods_peak_anomaly_flex_coarse.csv")
    ...
    nodal_seasonality, nodal_flex_periods, nodal_flex_anomaly_periods, nodal_peak_anomaly_flex = nodal_flex_periods_seasonality(nodal_flex_u_xr, nodes, years, periods)
    ...
    disp_tech = ["biomass", "nuclear", "H2 fuel cell", "battery discharger", "PHS", "hydro"]
    all_system_anomaly = compute_system_anomaly(periods, years, total_load, wind_cf, solar_cf, avg_load, avg_wind, avg_solar, opt_networks)
    all_used_flexibility = compute_used_flex(periods, nodal_flex_p, nodal_flex_periods, disp_tech)
    all_flex_anomaly = compute_used_flex(periods, nodal_flex_p, nodal_flex_anomaly_periods, disp_tech)
